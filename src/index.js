...
if (
  ...
  try {
    ...
    return json({
      ...
      if (
  url.pathname === '/api/bsc/financeiro-produto' &&
  request.method === 'GET'
) {
  const permitido = await podeConsultarModulo(
    request,
  ...
  try {
    const resultado = await listarBscFinanceiroProduto(request, env);
    ...
    return json({
      error:error.message ||
        'Não foi possível consultar o financeiro por produto.'
    }, 502);
  }
}

if (
  url.pathname === '/api/bsc/devolucoes-financeiro-produto' &&
  request.method === 'GET'
) {
  const permitido = await podeConsultarModulo(
    request,
  ...
  try {
    const resultado = await listarBscDevolucoesFinanceiroProduto(
      request,
      env
    );
...
if (
    ...
    return json({
      error:error.message ||
        'Não foi possível consultar as devoluções por vendedor.'
            ...
            if (
  url.pathname === '/api/bsc/vendedor-produto' &&
  request.method === 'GET'
) {
  const permitido = await podeConsultarModulo(
    request,
  ...
  try {
    const resultado = await listarBscVendedorPorProduto(
      request,
      env
    );
